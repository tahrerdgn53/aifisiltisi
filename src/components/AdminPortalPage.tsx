      ...
      <div className="max-w-4xl mx-auto relative z-10">
        <div className="flex items-center justify-between border-b border-white/5 pb-6 mb-8">
          <div className="flex items-center space-x-3.5">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight flex items-center space-x-2">
                ...
                <span className="text-[9px] bg-cyan-500/10 text-cyan-400 border border-cyan-500/25 px-2.5 py-0.5 rounded-full font-bold">
                  RESTRİKTE
                </span>
              </h1>

              <p className="text-xs text-slate-500 mt-1">
                Platform araçlarını, haber akışlarını ve içerikleri güvenli
                şekilde güncelleyin.
              </p>
            </div>
          </div>
          ...
          {isAdmin && (
            <button
              onClick={onLogout}
              className="flex items-center space-x-1.5 px-4 py-2 bg-rose-500/15 hover:bg-rose-500/20 border border-rose-500/20 text-rose-400 text-xs font-bold rounded-xl transition cursor-pointer"
            >
          ...
          <div className="max-w-md mx-auto py-12 text-center space-y-6">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-bold text-white tracking-tight">
                Güvenli Katman Şifre Doğrulaması
              </h3>
          ...
          <div className="space-y-8">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              <div className="p-4 rounded-2xl bg-white/[0.01] border border-white/5">
                ...
                <span className="text-xl sm:text-2xl font-extrabold text-white mt-1.5 block font-mono">
                  {tools.length} adet
                </span>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-white/[0.01] border border-white/5 col-span-2 sm:col-span-1">
                ...
                <span className="text-xs font-bold text-emerald-400 mt-2.5 block font-mono">
                  Aktif ve Korumalı
                </span>
              </div>
            </div>

            <div className="flex border-b border-white/5 space-x-2 overflow-x-auto">
              <button
                onClick={() => setActiveSubTab('create')}
                className={`py-2.5 px-4 text-xs sm:text-sm font-bold transition relative whitespace-nowrap ${
                  activeSubTab === 'create'
            ...
            <div className="bg-[#040811] p-6 sm:p-8 rounded-3xl border border-white/5 shadow-xl">
              {activeSubTab === 'create' && (
                <div className="space-y-4">
                  <h3 className="text-sm font-bold text-white tracking-wider uppercase font-mono pb-2 border-b border-white/5">
                    YENİ ARAÇ VEYA HABER YAYINLA
                  </h3>
              ...
              {activeSubTab === 'manage_tools' && (
                <div className="space-y-4">
                  <h3 className="text-sm font-bold text-white tracking-wider uppercase font-mono pb-2 border-b border-white/5">
                    AI ARAÇLARINI LİSTEDEN SİL / DÜZENLE
                  </h3>
                        ...
                        <div className="flex items-center justify-between p-3.5 bg-white/[0.01] hover:bg-white/[0.02] border border-white/5 rounded-xl transition text-xs gap-3">
                          <div className="flex items-center space-x-3 truncate min-w-0">
                            ...
                            <div className="truncate">
                              ...
                              <span className="text-[10px] text-slate-500 font-mono tracking-wide block truncate">
                                Developer: {tool.developer || 'Belirtilmedi'}{' '}
                                &bull; Fiyat: {tool.pricing}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center space-x-2 flex-shrink-0">
                            <button
                              onClick={() =>
                                setEditingToolId(
                                  editingToolId === tool.id ? null : tool.id
                                )
                              }
                            ...
                            <button
                              onClick={() => {
                                if (
                                  window.confirm(
                                    `${tool.name} aracını kalıcı olarak silmek istediğinize emin misiniz?`
                                  )
                                ) {
              ...
              {activeSubTab === 'manage_news' && (
                <div className="space-y-4">
                  <h3 className="text-sm font-bold text-white tracking-wider uppercase font-mono pb-2 border-b border-white/5">
                    HABER MAKALE VE GÜNCELLEMELERİ YÖNET
                  </h3>
